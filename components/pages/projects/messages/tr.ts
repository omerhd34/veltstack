export const projectsPageTr = {
  heroBadge: "Portföy",
  heroTitle: "Gerçek iş hedeflerine hizmet eden projeler",
  heroSubtitle:
    "Finansal teknolojiden e-ticarete, kurumsal katalogdan sağlık platformlarına — her proje ölçülebilir sonuçlar ve kullanıcı deneyimi odaklı geliştirildi.",
  heroSubtitleSecondary:
    "Detaylı proje incelemeleriyle her projenin kapsamını, teknoloji yığınını ve teslim metriklerini inceleyebilirsiniz.",
  heroImageAlt:
    "HDMI kablo ile monitöre bağlı laptop ve ekranlarda kod yazan geliştirici çalışma alanı",
  scrollLabel: "Projeleri incele",
  stats: [
    { value: "4", label: "Seçilmiş Proje" },
    { value: "__techCount__", label: "Teknoloji" },
    { value: "100%", label: "Tasarım + Kod" },
    { value: "1-3", label: "Teslim Süresi(Ay)" },
  ],
  showcaseBadge: "Proje İncelemeleri",
  showcaseTitle: "Her proje, bir iş hikayesi",
  showcaseSubtitle:
    "Tasarım ve geliştirmeyi tek çizgide birleştiren, performans ve ölçülebilir sonuçlarla dengelenmiş projeler.",
  viewLabel: "Detayları incele",
  featuredLabel: "Öne Çıkan",
  approachBadge: "Yaklaşım",
  approachTitle: "Her projede aynı disiplin",
  approachSubtitle:
    "İş hedeflerini merkeze alan, uçtan uca düşünülmüş bir geliştirme süreci.",
  approachItems: [
    {
      title: "Keşif & Strateji",
      description:
        "İş hedeflerini, kullanıcı ihtiyaçlarını ve teknik kısıtları baştan netleştiririz.",
    },
    {
      title: "Tasarım & Geliştirme",
      description:
        "UI/UX ve kod aynı çizgide ilerler; performans ve erişilebilirlik önceliklidir.",
    },
    {
      title: "Teslim & Ölçüm",
      description:
        "Canlıya alım sonrası metrikler, SEO ve kullanıcı geri bildirimleriyle sürekli iyileştirme.",
    },
  ],
  ctaTitle: "Sizin projeniz sırada mı?",
  ctaSubtitle:
    "Ücretsiz görüşmede projenizi dinleyip kapsam, süre ve teknoloji yığınını birlikte belirleyelim.",
  ctaButton: "Görüşme Planla",
  ctaNote: "24 saat içinde dönüş",
} as const;
