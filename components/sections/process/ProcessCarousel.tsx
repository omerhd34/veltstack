...
export interface ProcessSlideItem {
  id: string;
  title: string;
  description: string;
  index: number;
...
interface ProcessCarouselLabels {
  prev: string;
  next: string;
  dot: string;
}
...
interface ProcessCarouselProps {
  items: ProcessSlideItem[];
  labels: ProcessCarouselLabels;
  className?: string;
}
...
export function ProcessCarousel({
  ...
  const dotCount = Math.max(1, items.length - perPage + 1);
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <div ref={viewportRef} className="w-full overflow-hidden">
        ...
        >
          {items.map((item) => {
            const Icon = processStepIcons[item.index - 1];
            if (!Icon) return null;
      ...
      <div className="mt-8 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          {Array.from({ length: dotCount }).map((_, dotIndex) => (
            <button
              key={`process-slide-dot-${dotIndex}`}
              type="button"
