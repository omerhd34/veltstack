export const navTr = {
  services: "Hizmetler",
  servicesAll: "Tüm Hizmetler",
  servicesMegaMenuIntro:
    "Web, uygulama, SEO ve bakım ihtiyaçlarını tek çatı altında yönetiyoruz. Kurumsal site, e-ticaret veya mobil uygulama fark etmez; ihtiyacınıza uygun çözümü birlikte şekillendiririz. Kapsamı baştan netleştirir, süreci şeffaf yürütürüz. Ölçülebilir sonuçlara odaklanır; performans, erişilebilirlik ve SEO'yu önceliklendiririz. Tasarım, geliştirme ve yayını birlikte planlarız. Her aşamada sizi bilgilendirir, net teslim takvimi sunarız. Tek muhatapla ilerler, karar süreçlerini sadeleştiririz. Yayın sonrası destekle sisteminizi güvenli ve güncel tutarız.",
  serviceWeb: "Web Sitesi",
  serviceWebDesc:
    "Kurumsal, e-ticaret ve portfolyo için tasarımdan yayına web geliştirme sunuyoruz. Performans, erişilebilirlik ve dönüşümü önceliklendirip mobil uyumlu altyapıyla yayına alıyoruz.",
  serviceRefresh: "Web & Uygulama Yenileme",
  serviceRefreshDesc:
    "Eski veya yavaş web ve mobil uygulamalarını modern, hızlı deneyimlere dönüştürüyoruz. Altyapı, arayüz ve içerik katmanlarını birlikte yenileyerek kesintisiz geçiş sağlıyoruz.",
  serviceApp: "Uygulama",
  serviceAppDesc:
    "iOS ve Android için performanslı, kullanıcı dostu mobil uygulamalar geliştiriyoruz. Mağaza yayını, güncelleme ve bakım sürecini baştan sona yönetiyor, akışları projenize göre kurguluyoruz.",
  serviceSeo: "SEO Hizmetleri",
  serviceSeoDesc:
    "Teknik altyapı, içerik stratejisi ve düzenli optimizasyonla organik büyüme sağlıyoruz. Ölçülebilir hedefler belirliyor, anahtar kelime çalışmalarını veriye dayalı bir plana dönüştürüyoruz.",
  serviceAudit: "Teknik Denetim",
  serviceAuditDesc:
    "Web sitesi, uygulama ve altyapınızı performans, güvenlik ve SEO açısından kapsamlı analiz ediyoruz. Öncelikli iyileştirme planı ve uygulanabilir adım listesi sunuyoruz.",
  serviceMaintenance: "Bakım & Destek",
  serviceMaintenanceDesc:
    "Güncelleme, yedekleme ve teknik destekle web ve uygulama varlıklarınızı güvenli tutuyoruz. Acil müdahale süreçlerini yönetiyor, izleme ritmini kurarak riskleri erken yakalıyoruz.",
  projectsAll: "Tüm Projeler",
  projectIqfinans: "IQfinansAI",
  projectYazici: "Yazıcı Ticaret | Profilo & LG",
  projectFablessi: "Fablessi",
  projectUzmanPsikolog: "Uzman Klinik Psikolog",
  projectPortfolio: "Portfolyo | Ömer Halis Demir",
  projectOnlinemuhasebe: "Online Muhasebe | Mali Müşavirlik",
  projectsMegaMenuIntro:
    "FinTech, e-ticaret, kurumsal ve sağlık gibi sektörlerde dijital ürünler geliştiriyoruz. Finans platformundan e-ticaret akışına, kurumsal vitrinden randevu sistemlerine kadar geniş bir yelpazede çalışıyoruz. İş hedeflerini baştan netleştirir, tasarım ve geliştirmeyi tek süreçte birleştiririz. Karmaşık gereksinimleri sade arayüzlere ve ölçülebilir sonuçlara dönüştürürüz. Performans, güvenlik ve SEO'yu yayın öncesinden planlarız. Her aşamada sizi bilgilendirir, net teslim takvimi sunarız. Tek muhatapla ilerler, karar süreçlerini sadeleştiririz. Yayın sonrası destekle ürünlerin canlı ve güncel kalmasını sağlarız. Her projede ölçülebilir iş sonuçlarını hedefler, uzun vadeli büyümeye uygun çözümler sunarız.",
  projectIqfinansDesc:
    "Yapay zekâ destekli finansal analiz platformu. Karanlık-aydınlık mod, Türkçe-İngilizce dil desteği ve karmaşık verileri anlaşılır panellere dönüştüren raporlama.",
  projectYaziciDesc:
    "Beyaz eşya ve elektronik kategorilerinde uçtan uca e-ticaret deneyimi kurduk. Ürün keşfinden ödeme ve sipariş takibine kadar dönüşüm odaklı bir satış altyapısı tasarladık.",
  projectFablessiDesc:
    "Bahçe ve balkon mobilyaları için premium kurumsal katalog. Türkçe-İngilizce dil desteği, zengin görsel sunum ve net kategori yapısıyla marka algısını güçlendiren vitrin.",
  projectUzmanPsikologDesc:
    "Uzman Klinik Psikolog Nisa Demir için profesyonel web platformu. Karanlık-aydınlık mod, randevu talebi, blog ve SEO odaklı kurumsal deneyim.",
  projectPortfolioDesc:
    "Full stack geliştirici portfolyo sitesi. Türkçe-İngilizce dil desteği, projeler, deneyimler ve hizmetler modern ve SEO uyumlu bir arayüzde sunuluyor.",
  projectOnlinemuhasebeDesc:
    "Mali müşavirlik ve muhasebe hizmetleri için kurumsal portfolyo sitesi. Karanlık-aydınlık mod, pratik bilgiler, resmi platform kısayolları ve iletişim formu.",
  projects: "Projeler",
  blog: "Blog",
  blogAll: "Tüm Yazılar",
  blogMegaMenuIntro:
    "Web geliştirme, mobil uygulama, SEO, hosting ve dijital dönüşüm üzerine pratik içerikler hazırlıyoruz. Gerçek projelerden öğrendiklerimizi sade bir dille paylaşıyor; teknik konuları anlaşılır sunuyoruz. Yazılım, altyapı ve yayın süreçlerinde sık sorulan sorulara net yanıtlar bulabilirsiniz. Her yazıda uygulanabilir adımlar, kontrol listeleri ve öneriler yer alır. Domain seçiminden Vercel'de yayına, SEO planlamasından özel yazılım kararlarına kadar geniş konuları ele alırız. İster yeni başlıyor olun ister ölçekleniyor olun; size yol gösterecek rehberler burada. Dijital büyüme yolculuğunuzda ihtiyaç duyduğunuz rehberliği burada bulabilirsiniz.",
  blogGelenekselYazilimDesc:
    "Kaliteli yazılımda insan geliştiricinin kod yazmasının neden şart olduğunu; AI'ın yardımcı araç olarak sınırlarını, risklerini ve doğru yaklaşımı detaylı biçimde anlatıyoruz.",
  blogOzelYazilimCmsDesc:
    "Özel yazılımın CMS'e göre neden şart olduğunu; kaliteli projede bir insanın kod yazmasının neden vazgeçilmez olduğunu örneklerle birlikte anlatıyoruz.",
  blogDomainHostingDesc:
    "Domain ve hosting kavramlarını açıklıyor; ikisinin farkını, birlikte nasıl çalıştıklarını ve domain satın alma sürecini adım adım, sade bir dille anlatıyoruz.",
  blogSeoDesc:
    "Arama motoru optimizasyonunun temellerini, türlerini ve uygulanabilir adımlarını kapsamlı bir rehberle paylaşıyoruz; teknik ve içerik tarafını birlikte ele alıyoruz.",
  blogVercelDeYayinDesc:
    "Next.js projelerini Vercel'e nasıl deploy edeceğinizi adım adım anlatıyor; ortam değişkenleri, özel domain, preview ortamları ve sık karşılaşılan hataları kapsıyoruz.",
  blogDogruDomainSecimiDesc:
    "Markanıza uygun domain adını nasıl belirleyeceğinizi, uzantı seçimini ve sık yapılan hataları pratik bir rehberle paylaşıyoruz; SEO etkisini de ele alıyoruz.",
  about: "Hakkımızda",
  contact: "İletişim",
  cta: "Proje Konuşalım",
  switchToEn: "İngilizce'ye geç",
  switchToTr: "Türkçe'ye geç",
} as const;
